    ...
    const auto result = nautilus::invoke(
        +[](double lonValue,
            ...
            try
            {
                ...
                while (!stboxWkt.empty() && (stboxWkt.front() == '\'' || stboxWkt.front() == '"'))
                {
                    stboxWkt.erase(stboxWkt.begin());
                }
                while (!stboxWkt.empty() && (stboxWkt.back() == '\'' || stboxWkt.back() == '"'))
                {
                    stboxWkt.pop_back();
                }

                if (temporalGeometryWkt.empty() || stboxWkt.empty())
                {
                    std::cout << "TGEO_AT_STBOX received empty arguments" << std::endl;
                    return 0;
                }

                MEOS::Meos::TemporalGeometry temporalGeometry(temporalGeometryWkt);
                MEOS::Meos::SpatioTemporalBox stbox(stboxWkt);
                MEOS::Meos::TemporalHolder clipped(tgeo_at_stbox(temporalGeometry.getGeometry(), stbox.getBox(), borderInclusiveFlag));
