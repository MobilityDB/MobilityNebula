...
# ===========================================================================
# Trgeometry operator templates (W148/W149 wave).
#
# Physical CPP builders for the 5 trgeometry spatial-predicate layouts
# plus 2 NAD variants. Called directly from emit_operator() instead of
# being .format()-expanded, because the C++ bodies contain literal braces.
#
# Logical HPP/CPP: two style variants ("compact" = Eintersects style,
# "nad" = NadTrgeometry / AlwaysEq / EverEq multi-line style).
# ===========================================================================
...
def _trgeo_phys_header(nebula_name, includes):
    return (
        _TRGEO_LICENSE + "\n\n"
        + f"#include <Functions/Meos/{nebula_name}PhysicalFunction.hpp>\n"
        + includes + "\n\n"
        + "extern \"C\" {\n"
        + "#include <meos.h>\n"
        + "#include <meos_geo.h>\n"
...
def _trgeo_reg_compact(nebula_name, n):
    """Registrar block: compact style (== without spaces, 34-space arg indent)."""
    pad = "                                  "
    args = "\n".join(
        pad + f"std::move(arguments.childFunctions[{i}])" + ("," if i < n - 1 else ")")
        for i in range(n)
    )
    return (
        f"\nPhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::"
        f"Register{nebula_name}PhysicalFunction(\n"
...
def _trgeo_reg_nad(nebula_name, n):
    """Registrar block: nad/multiline style (== with spaces, 34-space indent)."""
    pad = "                                  "
    args = "\n".join(
        pad + f"std::move(arguments.childFunctions[{i}])" + ("," if i < n - 1 else ")")
        for i in range(n)
...
def _trgeo_logical_hpp_compact(nebula_name, brief, ctor_args):
    ctor = ", ".join(f"LogicalFunction {a}" for a in ctor_args)
    return (
        _TRGEO_LICENSE + "\n\n"
        "#pragma once\n\n"
        "#include <Functions/LogicalFunction.hpp>\n"
...
def _trgeo_logical_hpp_nad(nebula_name, brief, ctor_args):
    ctor = ", ".join(f"LogicalFunction {a}" for a in ctor_args)
    return (
        _TRGEO_LICENSE + "\n\n"
        "#pragma once\n\n"
...
def _trgeo_logical_cpp_compact(nebula_name, ctor_args, invariants, ret="FLOAT64"):
    n = len(ctor_args)
    ctor = ", ".join(f"LogicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    parameters.push_back(std::move({a}));" for a in ctor_args)
    inv = "\n".join(
        f"    INVARIANT(c[{i}].getDataType().isType(DataType::Type::{t}), \"{nm} must be {t}\");"
        for i, t, nm in invariants
    )
    reg_args = "\n".join(
        "                                 std::move(arguments.children[" + str(i) + "])" +
        ("," if i < n - 1 else ")")
        for i in range(n)
    )
    return (
        _TRGEO_LICENSE + "\n\n"
        f"#include <Functions/Meos/{nebula_name}LogicalFunction.hpp>\n"
        "#include <DataTypes/DataType.hpp>\n"
        "#include <DataTypes/DataTypeProvider.hpp>\n"
        "#include <DataTypes/Schema.hpp>\n"
        "#include <ErrorHandling.hpp>\n"
        ...
        f"std::vector<LogicalFunction> {nebula_name}LogicalFunction::getChildren() const {{ return parameters; }}\n"
        f"LogicalFunction {nebula_name}LogicalFunction::withChildren(const std::vector<LogicalFunction>& children) const {{\n"
        ...
        f"    if (const auto* o=dynamic_cast<const {nebula_name}LogicalFunction*>(&rhs)) return parameters==o->parameters;\n"
        "    return false;\n"
        "}\n"
        ...
        f"    std::vector<LogicalFunction> c; c.reserve({n});\n"
...
def _trgeo_logical_cpp_nad(nebula_name, ctor_args, invariants, ret="FLOAT64"):
    n = len(ctor_args)
    ctor = ", ".join(f"LogicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    parameters.push_back(std::move({a}));" for a in ctor_args)
    inv = "\n".join(
    ...
    return (
        ...
        f"bool {nebula_name}LogicalFunction::operator==(const LogicalFunctionConcept& rhs) const\n"
        "{\n"
        f"    if (const auto* other = dynamic_cast<const {nebula_name}LogicalFunction*>(&rhs))\n"
        "        return parameters == other->parameters;\n"
        "    return false;\n"
        "}\n\n"
        ...
        f"LogicalFunction {nebula_name}LogicalFunction::withInferredDataType(const Schema& schema) const\n"
        "{\n"
        f"    std::vector<LogicalFunction> c;\n"
        f"    c.reserve({n});\n"
...
def _trgeo_physical_hpp(nebula_name, ctor_args):
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    return (
        _TRGEO_LICENSE + "\n\n"
        "#pragma once\n\n"
        "#include <Functions/PhysicalFunction.hpp>\n"
...
# ---------- Physical CPP builders --------------------------------------
# Each returns a complete file string for a specific layout.

def _trgeo_phys_cpp_trgeometry_geo(nebula_name, meos_call, ctor_args):
    """Layout A: trgeometry_geo — VariableSizedData cast, trgeometryinst_make, compact registrar."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
        _trgeo_phys_header(nebula_name, _TRGEO_PHYS_INCLUDES_VS)
        + f"{nebula_name}PhysicalFunction::{nebula_name}PhysicalFunction({ctor}) {{\n"
        ...
        + "    return VarVal(result);\n"
        + "}\n"
        + _trgeo_reg_compact(nebula_name, n)
    )


def _trgeo_phys_cpp_trgeometry_geo_nad(nebula_name, meos_call, ctor_args):
    """Layout B: trgeometry_geo nad-style — no VariableSizedData cast, trgeometryinst_make, nad registrar."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
        _trgeo_phys_header(nebula_name, _TRGEO_PHYS_INCLUDES_NO_VS)
        + f"{nebula_name}PhysicalFunction::{nebula_name}PhysicalFunction({ctor})\n"
        + "{\n"
        + f"    paramFns.reserve({n});\n"
        + pushes + "\n}\n\n"
        ...
        + "                TInstant* inst = trgeometryinst_make(gs_ref, pose, (TimestampTz)ts);\n"
        ...
        + "    return VarVal(result);\n"
        + "}\n"
        + _trgeo_reg_nad(nebula_name, n)
    )


def _trgeo_phys_cpp_geo_trgeometry(nebula_name, meos_call, ctor_args):
    """Layout C: geo_trgeometry — VariableSizedData cast, trgeometryinst_make, geo-first MEOS call."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
        ...
        + pushes + "\n}\n\n"
        + f"VarVal {nebula_name}PhysicalFunction::execute(const Record& record, ArenaRef& arena) const {{\n"
        + "    auto tgt_wkt = paramFns[0].execute(record, arena).cast<VariableSizedData>();\n"
        + "    auto ref_wkt = paramFns[1].execute(record, arena).cast<VariableSizedData>();\n"
        + "    auto x1 = paramFns[2].execute(record, arena).cast<double>();\n"
        + "    auto y1 = paramFns[3].execute(record, arena).cast<double>();\n"
        + "    auto theta1 = paramFns[4].execute(record, arena).cast<double>();\n"
        + "    auto ts1 = paramFns[5].execute(record, arena).cast<uint64_t>();\n"
        + "    const auto result = nautilus::invoke(\n"
...
def _trgeo_phys_cpp_geo_trgeometry_eq(nebula_name, meos_call, ctor_args):
    """Layout D: geo_trgeometry eq-style — trgeometryinst_make, gs_tgt-first build order, nad registrar."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
...
def _trgeo_phys_cpp_trgeometry_geo_with_dist(nebula_name, meos_call, ctor_args):
    """Layout E: trgeometry_geo_with_dist — 7 args, dist as 3rd MEOS arg."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
...
def _trgeo_phys_cpp_trgeometry_trgeometry(nebula_name, meos_call, ctor_args):
    """Layout F: trgeometry_trgeometry — 10 args, VariableSizedData cast, trgeometryinst_make."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
...
def _trgeo_phys_cpp_trgeometry_trgeometry_nad(nebula_name, meos_call, ctor_args):
    """Layout G: trgeometry_trgeometry nad-style — trgeometryinst_make, no cast, nad registrar."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
        ...
        + "                TInstant* inst1 = trgeometryinst_make(gs1, p1, (TimestampTz)ts1);\n"
        + "                free(gs1); free(p1);\n"
        + "                if (!inst1) return 0.0;\n"
        ...
        + "                TInstant* inst2 = trgeometryinst_make(gs2, p2, (TimestampTz)ts2);\n"
        + "                free(gs2); free(p2);\n"
        + "                if (!inst2) { free(inst1); return 0.0; }\n"
        + f"                int r = {meos_call}((Temporal*)inst1, (Temporal*)inst2);\n"
        + "                free(inst1); free(inst2);\n"
...
def _trgeo_phys_cpp_trgeometry_trgeometry_with_dist(nebula_name, meos_call, ctor_args):
    """Layout H: trgeometry_trgeometry_with_dist — 11 args, trgeometryinst_make."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
        ...
        + "    auto theta2 = paramFns[8].execute(record, arena).cast<double>();\n"
        + "    auto ts2 = paramFns[9].execute(record, arena).cast<uint64_t>();\n"
        + "    auto dist = paramFns[10].execute(record, arena).cast<double>();\n"
        + "    const auto result = nautilus::invoke(\n"
...
def _trgeo_phys_cpp_nad_trgeometry_geo(nebula_name, meos_call, ctor_args):
    """Layout I: nad_trgeometry_geo — double return, trgeometryinst_make, nad registrar."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
...
def _trgeo_phys_cpp_nad_trgeometry_trgeometry(nebula_name, meos_call, ctor_args):
    """Layout J: nad_trgeometry_trgeometry — 10 args, double return, trgeometryinst_make."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
...
# Invariant sets used by build_descriptor.py classifiers (exported here so
# emit_trgeometry_operator can reference them by name).
_INV_TRG_GEO_X1 = [(0,"VARSIZED","ref_wkt"),(1,"FLOAT64","x1"),(2,"FLOAT64","y1"),(3,"FLOAT64","theta1"),(4,"UINT64","ts1"),(5,"VARSIZED","tgt_wkt")]
_INV_TRG_GEO_X  = [(0,"VARSIZED","ref_wkt"),(1,"FLOAT64","x"),(2,"FLOAT64","y"),(3,"FLOAT64","theta"),(4,"UINT64","ts"),(5,"VARSIZED","tgt_wkt")]
_INV_GEO_TRG_X1 = [(0,"VARSIZED","tgt_wkt"),(1,"VARSIZED","ref_wkt"),(2,"FLOAT64","x1"),(3,"FLOAT64","y1"),(4,"FLOAT64","theta1"),(5,"UINT64","ts1")]
_INV_GEO_TRG_X  = [(0,"VARSIZED","tgt_wkt"),(1,"VARSIZED","ref_wkt"),(2,"FLOAT64","x"),(3,"FLOAT64","y"),(4,"FLOAT64","theta"),(5,"UINT64","ts")]
_INV_TRG_GEO_DIST = _INV_TRG_GEO_X1 + [(6,"FLOAT64","dist")]
_INV_TRG_TRG_X1 = [(0,"VARSIZED","ref1_wkt"),(1,"FLOAT64","x1"),(2,"FLOAT64","y1"),(3,"FLOAT64","theta1"),(4,"UINT64","ts1"),(5,"VARSIZED","ref2_wkt"),(6,"FLOAT64","x2"),(7,"FLOAT64","y2"),(8,"FLOAT64","theta2"),(9,"UINT64","ts2")]
_INV_TRG_TRG_DIST = _INV_TRG_TRG_X1 + [(10,"FLOAT64","dist")]

_TRGEO_PHYS_DISPATCH = {
    "build_trgeometry_geo":                   (_trgeo_phys_cpp_trgeometry_geo,                ["ref_wkt","x1","y1","theta1","ts1","tgt_wkt"],         _INV_TRG_GEO_X1, "compact"),
    "build_trgeometry_geo_nad":               (_trgeo_phys_cpp_trgeometry_geo_nad,            ["ref_wkt","x","y","theta","ts","tgt_wkt"],             _INV_TRG_GEO_X,  "nad"),
    "build_geo_trgeometry":                   (_trgeo_phys_cpp_geo_trgeometry,                ["tgt_wkt","ref_wkt","x1","y1","theta1","ts1"],          _INV_GEO_TRG_X1, "compact"),
    "build_geo_trgeometry_eq":                (_trgeo_phys_cpp_geo_trgeometry_eq,             ["tgt_wkt","ref_wkt","x","y","theta","ts"],              _INV_GEO_TRG_X,  "nad"),
    "build_trgeometry_geo_with_dist":         (_trgeo_phys_cpp_trgeometry_geo_with_dist,      ["ref_wkt","x1","y1","theta1","ts1","tgt_wkt","dist"],   _INV_TRG_GEO_DIST,"compact"),
    "build_trgeometry_trgeometry":            (_trgeo_phys_cpp_trgeometry_trgeometry,         ["ref1_wkt","x1","y1","theta1","ts1","ref2_wkt","x2","y2","theta2","ts2"],          _INV_TRG_TRG_X1, "compact"),
    "build_trgeometry_trgeometry_nad":        (_trgeo_phys_cpp_trgeometry_trgeometry_nad,     ["ref1_wkt","x1","y1","theta1","ts1","ref2_wkt","x2","y2","theta2","ts2"],          _INV_TRG_TRG_X1, "nad"),
    "build_trgeometry_trgeometry_with_dist":  (_trgeo_phys_cpp_trgeometry_trgeometry_with_dist,["ref1_wkt","x1","y1","theta1","ts1","ref2_wkt","x2","y2","theta2","ts2","dist"],  _INV_TRG_TRG_DIST,"compact"),
    "build_nad_trgeometry_geo":               (_trgeo_phys_cpp_nad_trgeometry_geo,            ["ref_wkt","x","y","theta","ts","tgt_wkt"],             _INV_TRG_GEO_X,  "nad"),
    "build_nad_trgeometry_trgeometry":        (_trgeo_phys_cpp_nad_trgeometry_trgeometry,     ["ref1_wkt","x1","y1","theta1","ts1","ref2_wkt","x2","y2","theta2","ts2"],          _INV_TRG_TRG_X1, "nad"),
}


def emit_trgeometry_operator(op, output_root: Path):
    """Emit 4 files for a trgeometry operator using the W148/W149 templates.

    op must have: nebula_name, meos_call, comment_one_liner, and one of the
    build_* keys from _TRGEO_PHYS_DISPATCH set to True.
    """
    nebula_name = op["nebula_name"]
    meos_call   = op["meos_call"]
    brief       = op.get("comment_one_liner", f"MEOS {meos_call} over a trgeometry instant.")

    build_key = next((k for k in _TRGEO_PHYS_DISPATCH if op.get(k)), None)
    if build_key is None:
        sys.stderr.write(f"  ! {nebula_name}: no trgeometry build key found\n")
        return

    phys_fn, ctor_args, invariants, log_style = _TRGEO_PHYS_DISPATCH[build_key]
    ...
    if log_style == "nad":
        logical_hpp_path.write_text(_trgeo_logical_hpp_nad(nebula_name, brief, ctor_args))
        logical_cpp_path.write_text(_trgeo_logical_cpp_nad(nebula_name, ctor_args, invariants))
    else:
        logical_hpp_path.write_text(_trgeo_logical_hpp_compact(nebula_name, brief, ctor_args))
        logical_cpp_path.write_text(_trgeo_logical_cpp_compact(nebula_name, ctor_args, invariants))

    physical_hpp_path.write_text(_trgeo_physical_hpp(nebula_name, ctor_args))
    physical_cpp_path.write_text(phys_fn(nebula_name, meos_call, ctor_args))

    sys.stderr.write(f"  ✓ {nebula_name} [trgeometry/{build_key}]: emitted 4 files\n")
...
def generic_fields(op):
    ...
    fields = [tuple(f) for f in input_spec(op)["fields"]]
...
def input_spec(op):
    """The operand builder for `op`, catalog-stated where one exists.

    A descriptor carrying its own `input_spec` names the MEOS constructor the
    catalog states for that temporal type, and it WINS over the table below,
    which is left holding only the composite operands -- a geometry, a circular
    buffer, a pose, a network point -- whose decomposition into stream columns
    is this binding's own choice and not a fact any catalog makes.
    """
    spec = op.get("input_spec")
    if not spec:
        return GENERIC_INPUTS[op["input_type"]]
    if "build" in spec:
        return spec
    if spec.get("parser"):
        # A box operand arrives as the text its own `_out` writes, so it is read
        # back with the parser the catalog names for that type. The quote
    ...
    cast = f'({spec["cast"]}) ' if spec.get("cast") else ""
    return dict(
        spec,
        header="meos.h",
        build=(
            '                TimestampTz {var}Ts = timestamptz_in('
            'MEOS::Meos::convertEpochToTimestamp(ts).c_str(), -1);\n'
            f'                Temporal* {{var}} = {cast}{spec["ctor"]}(value, {{var}}Ts);\n'
            '                if (!{var}) return {z};\n'))


def emit_operator(op, output_root: Path):
    nebula_name = op["nebula_name"]
    # Trgeometry operators use dedicated per-layout builders (W148/W149 wave).
    # Detected by any of the 10 trgeometry build keys registered in _TRGEO_PHYS_DISPATCH.
    if any(op.get(k) for k in _TRGEO_PHYS_DISPATCH):
        emit_trgeometry_operator(op, output_root)
        return
...
GENERIC_RETURNS = {
    ...
    # The accessor is resolved per operator from the catalog and carried in the
    # descriptor, so these name the marshalling only.
    "extract_int":    ("int", "INT32", "0", None),
    "extract_double": ("double", "FLOAT64", "0.0", None),
    "extract_bool":   ("bool", "BOOLEAN", "false", None),
    # A Temporal*-returning transform whose result is serialized back to hex-WKB
    # and emitted as a VARSIZED field (the cross-operator exchange form). The
    # assembler picks GENERIC_PHYSICAL_WKB_TEMPLATE for it; the operand and the
    # extra arguments are assembled exactly as for a scalar-returning operator.
...
def assemble_generic_physical(op):
    ...
    A temporal-returning operator differs from a scalar-returning one only in
    what it does with the RESULT -- it hands the value on in the hex-WKB
    exchange form instead of reading a number out of it. Its operand and its
    extra arguments are assembled exactly the same way, which is why both go
    through this one function. The separate assembler that used to answer for
    the temporal-returning half wrote a FIXED two-hex-WKB-operand call for
    every operator it touched, so a unary transform and a scalar-argument one
    were emitted with an argument they do not take, each disagreeing with the
    arity its own generated header declares.
    """
    name = op["nebula_name"]
    wkb = op["return_kind"] == "wkb"
    inp = input_spec(op)
    if wkb:
        ret_cpp, zero, extract_fn = "char*", "(char*) nullptr", None
    else:
        ret_cpp, _, zero, extract_fn = GENERIC_RETURNS[op["return_kind"]]
        # the descriptor resolves which MEOS accessor reads the instant's value
        # from the catalog, so it wins over anything named here
        extract_fn = op.get("extract_fn", extract_fn)
    extras = op.get("extra_args", [])

    # Ordered (lambda-param) fields: primary input fields, then each extra arg's.
    fields = [tuple(f) for f in inp["fields"]]
    headers = {"meos.h", inp["header"]}
    # the header declaring the operand's own constructor, as the catalog states it
    ...
    template = GENERIC_PHYSICAL_WKB_TEMPLATE if wkb else GENERIC_PHYSICAL_TEMPLATE
...
{casts}
    ...
    // The MEOS call and the serialization of its result run inside ONE invoke,
    // which returns the heap hex string; the bytes are copied into an
    // arena-allocated VariableSizedData below. Operands and result are freed here.
...
def _dispatch_case_current(op):
    """Parser dispatch case in the CURRENT AntlrSQLParser style (visit()/return
    LogicalFunction), matching the live W2..W147 chain. Generic over arity.
    Returns the final C++ string (no further .format). END marker only (the
    live style carries no BEGIN marker)."""
    build_key = next((k for k in _TRGEO_PHYS_DISPATCH if op.get(k)), None)
    n = len(_TRGEO_PHYS_DISPATCH[build_key][1]) if build_key else len(op.get("args", []))
    tok, name = op["sql_token"], op["nebula_name"]
    visits = "\n".join(
        f"            auto arg{i} = visit(ctx->functionParam({i})).as<LogicalFunction>();"
        for i in range(n))
    ctor_args = ", ".join(f"std::move(arg{i})" for i in range(n))
    return f"""        case AntlrSQLParser::{tok}: {{
            PRECONDITION(ctx->functionParam().size() == {n},
                         "{name} requires {n} args but got {{}}",
                         ctx->functionParam().size());
{visits}
            return LogicalFunction({name}LogicalFunction({ctor_args}));
        }}
...
def _generic_dispatch_case(op):
    ...
    When op carries alias_sql_token an empty fall-through case is emitted for
    the alias immediately before the primary case so both grammar tokens route
    to the same LogicalFunction constructor body:

        case AntlrSQLLexer::E_EQ_TFLOAT_FLOAT:    // primary
        case AntlrSQLLexer::EVER_EQ_TFLOAT_FLOAT: // alias — falls through
        { ... }
    """
    n = len(op["args"])
    tok, name = op["sql_token"], op["nebula_name"]
    alias = op.get("alias_sql_token")
    alias_line = f"\n        case AntlrSQLLexer::{alias}: // alias: {tok}" if alias else ""
    pops = "\n".join(
    ...
    return f"""        /* BEGIN CODEGEN PARSER GLUE: {tok} */
        case AntlrSQLLexer::{tok}:{alias_line}
        {{
            const auto argCount = context->expression().size();
            if (argCount != {n})
...
def inject_cmake_entries(operators, output_root: Path) -> int:
    ...
    for layer in ("logical", "physical"):
        ...
        if new_lines:
            block = "\n".join(new_lines) + "\n"
            # The physical Meos CMakeLists wraps its add_plugin calls in
            # `if(NES_ENABLE_MEOS) ... endif()`; new entries MUST land inside the
            # guard (before the final endif) so they inherit the MEOS plugin
            # include dir that provides MEOSWrapper.hpp. The logical layer has no
            # guard, so it appends at EOF.
            idx = body.rfind("\nendif()")
            if idx != -1:
                insert_at = idx + 1  # just before the final `endif()` line
                body = body[:insert_at] + block + body[insert_at:]
                cml.write_text(body)
            else:
                with cml.open("a") as f:
                    f.write(block)
            sys.stderr.write(f"  ✓ cmake-entries ({layer}): added {len(new_lines)} entry(ies)\n")
            n_added += len(new_lines)
    return n_added


def _op_tokens(op: dict) -> list:
    """Return all grammar tokens for *op*: primary sql_token first, then alias.

    When build_descriptor.py emits alias_sql_token (sqlfn-backed canonical
    naming with the raw fn.upper() kept as backward-compat alias) both tokens
    must appear in the lexer rules and the functionName alternation so the
    parser accepts either spelling and routes to the same LogicalFunction.
    """
    tokens = [op["sql_token"]]
    alias = op.get("alias_sql_token")
    if alias:
...
def inject_g4(operators, g4_path: Path) -> int:
    ...
    # When an op carries alias_sql_token both the primary and the alias get a
    # lexer rule so the parser accepts either spelling.
    new_tokens = []
    for op in operators:
        for tok in _op_tokens(op):
    ...
    # Both the primary token and any alias_sql_token are added so the grammar
    # accepts either spelling as a function name.
...
def inject_parser_cpp(operators, cpp_path: Path) -> int:
    ...
    for op in operators:
        ...
        end_marker = f"/* END CODEGEN PARSER GLUE: {op['sql_token']} */"
        if marker in body or end_marker in body:
            continue
        # Also skip if a pre-existing hand-written case for this token already
        # exists (no marker, but a `case AntlrSQL{Lexer,Parser}::TOKEN:` line is present).
        # Check both primary token and alias so we don't duplicate an already-registered pair.
        def _has_case(tok):
            return bool(re.search(rf"case\s+AntlrSQL(?:Lexer|Parser)::{re.escape(tok)}\s*:", body))
        if _has_case(op["sql_token"]):
            sys.stderr.write(
            ...
            continue
        alias = op.get("alias_sql_token")
        if alias and _has_case(alias):
            sys.stderr.write(
                f"  ! parser-cpp: pre-existing hand-written case for alias {alias} detected; "
                f"skipping codegen injection (will not duplicate)\n"
            )
            continue
        if any(op.get(k) for k in _TRGEO_PHYS_DISPATCH):
            case_str = _dispatch_case_current(op)          # current AntlrSQLParser style; final string
        elif op.get("build_generic"):
